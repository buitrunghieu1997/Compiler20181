...
public class HandleFlow {
  ...
  public static void printFlow() {
    ...
    for (int i = functionsList.size() - 1; i >= 0; i--) {
      ...
      } else if (temp.contains("ifstmt")) System.out.print(temp.replace("ifstmt", "") + "  ->  (");
      else {
  ...
  public static String getNextString() {
    if (functionsList.get(functionListCount) == null) return null;
  ...
  public static boolean isParent(String str) {
    ...
    for (int i = 0; i < parentFunctionList.size(); i++)
      if (str.equals(parentFunctionList.get(i).functionName)) return true;
    return false;
  }
}
